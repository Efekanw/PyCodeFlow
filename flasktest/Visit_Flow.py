...
class Visit_Floww(Visitor_Recursive):
    ...
    def return_counters(self):
        ...
        #list_counters['count_if'] = self.count_if
        ...
        # list_counters['count_pop'] = self.count_pop
        return list_counters

    def assign_counters(self, list_counters):
        self.counter_if = list_counters['if']
        #self.count_if = list_counters['count_if']
        ...
        # self.count_pop = list_counters['count_pop']
    ...
    def file_input(self, tree):
        if self.subgraph_def == 0:  # -2 ye bak
            self.get_file_lines()
    ...
    def import_stmt(self, tree):
        try:
            while self.count_def > 1 and self.valid == 0 or (
                    self.subgraph == 0 and self.count_def > 0):  # if ve diğer nodelara eklenemli
    ...
    def funcdef(self, tree):
        try:
            ...
            if self.subgraph == 1 and self.valid == 0:
                ...
                self.valid = 0
                n = 'def ' + str(self.count_def) + ' ' + str(self.count_if)
                if self.count_def > 1 and self.subgraph == 1: ## and sub kontrolü
        ...
        try:
            ...
            if self.subgraph == 1 and self.valid == 0:
                ...
                if text == self.func_name:
                    ...
                    self.A.add_edge(self.node, self.func_name)
                    self.stack_stmt.append('class ' + str(self.count_def) + ' ' + str(self.count_if))
                    self.stack_pos.append(tree.meta.end_pos)
                    print("maingraph")
                    print(self.A.string())
                ...
                self.valid = 0
                n = 'class ' + str(self.count_class) + ' ' + str(self.count_if)
                self.stack_pos.append(tree.meta.end_pos)
                self.stack_stmt.append('class ' + str(self.count_class) + ' ' + str(self.count_if))
                self.stmt_pop(tree, n)
        except:
            print('class error')
            ...
            if self.valid == 1:
                ...
                if n.startswith('elifs'):
                    ...
                    while len(self.stack_pos) > 0 and self.stack_pos[-1] <= tree.meta.start_pos:
                        ...
                        if stmt == 'class ' + str(self.count_class) + ' ' + str(self.count_if):
                        ...
                        if stmt.startswith('except'):
                            self.A.add_edge(n, stmt, label='Yes') # no
                            index = self.A.nodes().index(stmt) - 2 # -1 di
                        ...
                        if stmt.startswith('while'):
                            ...
                            end_while = 'end_while ' + counter + ' ' + indent
                            self.A.add_node(n=end_while, label='END', shape='ellipse')
                            if node in self.dict_break.keys():
                                self.A.add_edge(self.dict_break[node], end_while, label='break')###
                            if node in self.dict_continue.keys():
                                self.A.add_edge(self.dict_continue[node], node, label='continue')
                                self.break_contine= 1
                            self.A.add_edge(node, end_while, label='False')
                            ...
                            if node_for != '':
                                node_f = node_for
                            if n.startswith('for') or n.startswith('while'):  # or while
                                indent_n = n.split()[2]
                                if int(indent) == int(indent_n):
                                    self.A.add_edge(end_while, n, label='Try Next Item') # next for
                                elif int(indent) > int(indent_n) and int(indent) <= int(indent_for): ####################### TEST LAZIM
                                    self.A.add_edge(end_while, n,
                                                    label='Try Next Item') # next for
                                elif int(indent) > int(indent_for) and int(indent_for) != -1: # indeent for vardı
                                    self.A.add_edge(end_while, node_f,
                                                    label='Try Next Item')
                                ...
                                if int(indent_node) == int(indent):
                                    self.A.add_edge(end_while, n, label='Try Next Item')
                                elif int(indent_node) < int(indent):
                                    self.A.add_edge(end_while, node_for, label='Try Next Item')
                                ...
                                if n.startswith('elif_') or n.startswith('else'):
                                    ...
                                    self.dict_true[str(int(indent) - 1)].append(end_while)
                                else:
                                    self.A.add_edge(end_while, n)
                                if self.count_pop_for < 2:
                                    #index = self.A.nodes().index(n)
                                    ...
                                    if len(n.split()) == 1 or n.startswith('elif_') or n.startswith('if') or n.startswith('for')  or n.startswith('while') :
                        ...
                        if stmt.startswith('for'):
                            ...
                            node = self.A.get_node(stmt)
                            counter=node.split()[1]
                            end_for = 'end_for ' + counter + ' ' +indent
                            self.A.add_node(n=end_for, label='END', shape='ellipse')
                            if node in self.dict_break.keys():
                                self.A.add_edge(self.dict_break[node], end_for)
                            if node in self.dict_continue.keys():
                                self.A.add_edge(self.dict_continue[node], node, label='continue')
                                self.break_contine = 1
                            self.A.add_edge(node, end_for, label='No')
                            i = -1
                            c = 0
                            node_for = ''
                            indent_for = -1
                            if len(self.stack_stmt) > 0:
                                node_for = self.stack_stmt[i]
                                indent_for = node_for.split()[2]
                                while len(self.stack_stmt) > c and (not self.stack_stmt[i].startswith('for') and not self.stack_stmt[i].startswith('while')): # or while
                                    i -= 1
                                    c += 1
                                    try:
                                        node_for = self.stack_stmt[i]
                            ...
                            if n.startswith('for') or n.startswith('while'):
                                ...
                                if self.count_pop_for < 2:
                                    ...
                                    self.A.add_edge(node, stmt,
                                                    label='Try Next Item')
                            elif node_for.startswith('for') or node_for.startswith('while') and int(indent) > int(indent_for):
                                if int(indent_node) == int(indent):
                                    self.A.add_edge(end_for, n, label='Try Next Item')
                                elif int(indent_node) < int(indent):
                                    self.A.add_edge(end_for, node_for, label='Try Next Item')
                                if self.last_node_loop != stmt:
                                    if n.startswith('else'):
                                        index = -2
                                    else:
                                ...
                                if n.startswith('elif_') or n.startswith('else'):
                                    if str(int(indent)-1) not in self.dict_true.keys():
                                        self.dict_true[str(int(indent)-1)] = []
                                    self.dict_true[str(int(indent)-1)].append(end_for)
                                else:
                                    self.A.add_edge(end_for, n)
                                if self.count_pop_for < 2:
                                    index = -1
                                    node = self.A.nodes()[index -1]
                                    if len(n.split()) == 1 or n.startswith('elif_') or n.startswith(
                                            'if') or n.startswith('for') or n.startswith('while'):
                                        node = self.A.nodes()[index - 1 - 1]
                                    else:
                                        node = self.A.nodes()[index - 1]
                                    subgraph_node_len = 0
                ...
                if not n.startswith('elifs') and sub_in_sub == 0:
                    ...
                    if len(self.stack_stmt) > 0 and self.stack_stmt[-1].startswith('else') and self.flag_else == 1:
                        ...
                        self.last_pop = ''
                        self.new_else = n
                        self.dict_else_nodes[str(self.count_if - 1)] = self.new_else
                        self.index_dict_false = -1
                        ...
                        if len(self.stack_pos) > 0 and self.stack_pos[-1] > tree.meta.start_pos and prevnode.startswith('except'):
                            self.A.add_edge(prevnode, n, label='No') # yes
                               ...
                               prevnode  == 'for ' + str(self.count_for) + ' ' + str(self.count_if - 1) or prevnode == 'while ' + str(self.count_while) + ' ' + str(self.count_if - 1)): #  self.A.nodes()[-2
                            ...
                            if not self.last_pop.startswith('elif_'):
                             ...
                             self.last_pop = ''
                        elif  not self.A.nodes()[-1].startswith('end') and (self.last_pop != 'elif_ ' + str(self.counter_if) + ' ' + str(self.count_if - 1) and n != 'elif_ ' + str(self.counter_if) + ' ' + str(self.count_if - 1)): # and di
                            if self.is_sub == 1:
                                self.A.add_edge(node, n)
                                self.is_sub = 0
                            else:
        ...
        try: # func kontrolü
            ...
            if self.valid == 1:
                ...
                if stmt_type == 'assign':
                    ...
                    self.count_try += 1
                    n = stmt_type + ' ' + str(self.count_try) + ' ' + str(self.count_if)
                    ...
                    if f == 1:
                        ...
                        n = stmt_type + ' ' + str(self.count_try) + ' ' + str(self.count_if - 1)
                    ...
                    if f==1:
                        indent_pop = node_pop.split()[2]
                        n = stmt_type + ' ' + str(self.count_for) + ' ' + indent_pop
                    else:
                        n = stmt_type + ' ' + str(self.count_for) + ' ' + str(self.count_if)
                    ...
                    if f == 1:
                        indent_pop = node_pop.split()[2]
                        n = stmt_type + ' ' + str(self.count_while) + ' ' + indent_pop
                    else:
                        n = stmt_type + ' ' + str(self.count_while) + ' ' + str(self.count_if)
                    self.A.add_node(n=n, label=text, shape='hexagon')
                    self.counter_loop += 1
                    self.stmt_pop(tree, n)
                    self.count_if += 1
                    ...
                    text = 'break'
                    n = stmt_type + ' ' + str(self.count_break) + ' ' + str(self.count_if)
                    self.stmt_pop(tree, n)
                    i = -1
                    node_loop = ''
                    while (not self.A.nodes()[i].startswith('for') and not self.A.nodes()[i].startswith('while')) or (self.A.nodes()[i].startswith('for') and  self.A.nodes()[i].startswith('while') and (self.count_if == node_loop.split()[2]) ):
                        i -= 1
                    ...
                    text = 'continue'
                    n = stmt_type + ' ' + str(self.count_continue) + ' ' + str(self.count_if)
                    self.stmt_pop(tree, n)
                    i = -1
                    node_loop = ''
                    while not self.A.nodes()[i].startswith('for') and not self.A.nodes()[i].startswith('while'):
    ...
    def suite(self, tree):
        try:
            ...
            if len(self.stack_stmt) > 0 and ((self.stack_stmt[-1].startswith('for') or self.stack_stmt[-1].startswith('while'))):
                pad_indent = 1
            if (self.valid == 1 and 'elif_ ' + str(self.counter_if) + ' ' + str(self.count_if - 1 - pad_indent) in self.stack_stmt) or (len(self.stack_stmt) > 0 and (self.valid == 1 and self.stack_stmt[-1] == 'elifs ' + str(self.counter_if) + ' ' + str(self.count_if))): #or (self.subgraph == 1 and self.count_def > 0 and self.valid == 0):
                if 'elif_ ' + str(self.counter_if) + ' ' + str(self.count_if - 1 - pad_indent) in self.stack_stmt:
                    index = self.stack_stmt.index('elif_ ' + str(self.counter_if) + ' ' + str(self.count_if - 1 - pad_indent))
        ...
        try:
            ...
            if self.valid == 1:
                ...
                if f == 1:
                    ...
                    n = 'if' + ' ' + str(self.counter_if) + ' ' + str(self.count_if)
                self.counter_if += 1
                self.A.add_node(n=n, label=text, shape='diamond')
                self.stmt_pop(tree, n)
                self.count_if += 1
